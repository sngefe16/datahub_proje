...
class FifthViewLightGBM:
    """
    LightGBM model optimized for fifth view features.
    Enhanced with higher capacity for AUC-ROC > 0.95 and Recall > 0.9.
    Includes hyperparameter tuning support with Optuna/Hyperopt.
    """
    
    def __init__(self, 
                 n_estimators: int = 3000,  # Increased from 2000
                 max_depth: int = 12,  # Increased from 7
                 learning_rate: float = 0.01,
                 num_leaves: int = 127,  # Increased from 31
...
def optimize_hyperparameters_optuna(X_train, y_train,
    ...
    def objective(trial):
        params = {
            ...
            'metric': 'auc',
            'boosting_type': 'gbdt',
            'num_leaves': trial.suggest_int('num_leaves', 127, 511),  # Increased range for higher capacity (127-511)
            'learning_rate': trial.suggest_float('learning_rate', 0.001, 0.05, log=True),  # Lower min for more capacity
            'feature_fraction': trial.suggest_float('feature_fraction', 0.6, 1.0),
            'bagging_fraction': trial.suggest_float('bagging_fraction', 0.6, 1.0),
            'bagging_freq': trial.suggest_int('bagging_freq', 1, 7),
            'min_child_samples': trial.suggest_int('min_child_samples', 5, 100),
            'max_depth': trial.suggest_int('max_depth', 12, 20),  # Increased range for higher capacity (12-20)
    ...
    study = optuna.create_study(direction='maximize', study_name='fifth_view_optuna')
...
class EnsembleModel:
    ...
    Enhanced for fifth view with higher capacity models.
    """
    
    def __init__(self,
                 lgb_model: Optional[FifthViewLightGBM] = None,
        ...
        Parameters
        ----------
        lgb_model : FifthViewLightGBM, optional
...
def train_fifth_view_model(X_train, y_train,
                           X_val=None, y_val=None,
                           ...
                           cv_folds: int = 5) -> FifthViewLightGBM:
    """
    Train fifth view model with optional hyperparameter tuning.
    Enhanced with higher capacity for AUC-ROC > 0.95 and Recall > 0.9.
    ...
    model = FifthViewLightGBM(**model_params)
...
def train_ensemble_model(X_train, y_train,
    ...
    if use_lgb:
        ...
        lgb_model = train_fifth_view_model(
            X_train, y_train,
            X_val=X_val, y_val=y_val,
    ...
    if use_xgb and XGBOOST_AVAILABLE:
        ...
        if X_val_xgb is not None and y_val is not None:
            ...
            xgb_model = xgb.train(
                xgb_params,
                xgb_train,
                num_boost_round=xgb_params.get('n_estimators', 3000),
    ...
    if use_cat and CATBOOST_AVAILABLE:
        ...
        if cat_params is None:
            cat_params = {
                'iterations': 4000,  # Increased for higher capacity (targeting AUC-ROC > 0.98)
                'learning_rate': 0.01,
                'depth': 7,
    ...
    if weights is None and use_cv_for_weights:
            ...
            class XGBoostCVWrapper:
                ...
                def fit(self, X, y, categorical_features=None):
                    ...
                    self.model = xgb.train(
                        self.xgb_params,
                        xgb_data,
                        num_boost_round=self.xgb_params.get('n_estimators', 3000),
                        verbose_eval=False
                    )
        ...
        if len(models_list) > 0:
            from src.fifth_view.evaluation import compute_ensemble_weights_from_cv
