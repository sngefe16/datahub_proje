...
from src.fifth_view.data_loader import load_fifth_view_data
from src.fifth_view.preprocessing import preprocess_fifth_view_data, apply_smote
from src.fifth_view.feature_engineering import create_all_fifth_view_features
from src.fifth_view.feature_selection import comprehensive_feature_selection
from src.fifth_view.models import train_fifth_view_model, train_ensemble_model
from src.fifth_view.evaluation import (
    evaluate_model, 
...
def main(threshold: float = 0.65,
         ...
         use_ensemble: bool = True,  # Default True for fifth view
         use_smote: bool = True,  # New: SMOTE for class imbalance
         use_cross_validation: bool = False,  # New: Cross-validation
         cv_folds: int = 5,  # New: Number of CV folds
         min_recall: float = 0.9):
    """
    Main training pipeline for fifth view model.
    Targets AUC-ROC > 0.95 and Recall > 0.9.
    ...
    """
    print("=" * 70)
    print("Fifth View Model Training Pipeline")
    print("=" * 70)
    print("Targets: AUC-ROC > 0.95, Recall > 0.9")
    print("=" * 70)
    ...
    # 1. Load data
    print("\n[1/11] Loading data...")
    train_df, test_df = load_fifth_view_data(sample_size=None)  # Use all data
    
    # 2. Preprocessing
    print("\n[2/11] Preprocessing data...")
    train_df = preprocess_fifth_view_data(train_df, is_train=True)
    test_df = preprocess_fifth_view_data(test_df, is_train=False)
    
    # 3. Feature engineering
    print("\n[3/11] Creating features...")
    train_df = create_all_fifth_view_features(train_df, is_train=True)
    test_df = create_all_fifth_view_features(test_df, is_train=False)
    ...
    if use_feature_selection:
        ...
        from src.fifth_view.models import FifthViewLightGBM
        temp_model = FifthViewLightGBM(n_estimators=200, learning_rate=0.1, verbose=-1)
    ...
    if use_ensemble:
        ...
        model = train_fifth_view_model(
            X_train_split, y_train_split,
            X_val=X_val, y_val=y_val,
            categorical_features=categorical_features,
            model_params={
                'n_estimators': 4000,  # Increased for higher capacity (targeting AUC-ROC > 0.98)
                'learning_rate': 0.01,
    ...
    submission_path = project_root / "results" / "submission_fifth_view.csv"
    ...
    print("\n[12/12] Summary")
    print("=" * 70)
    print(f"Model: Fifth View {'Ensemble' if use_ensemble else 'LightGBM'}")
...
if __name__ == "__main__":
    ...
    model, metrics, threshold_results, optimal_threshold = main(
        ...
        n_trials=100,  # Increased for better hyperparameter tuning
        use_feature_selection=True,
        use_ensemble=True,  # Default True for fifth view (better performance)
