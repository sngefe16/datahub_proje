...
from src.fourth_view.data_loader import load_fourth_view_data
from src.fourth_view.preprocessing import preprocess_fourth_view_data
from src.fourth_view.feature_engineering import create_all_fourth_view_features
from src.fourth_view.feature_selection import comprehensive_feature_selection
from src.fourth_view.models import train_fourth_view_model, train_ensemble_model
from src.fourth_view.evaluation import (
    evaluate_model, 
    evaluate_multiple_thresholds,
    find_optimal_threshold,
    find_threshold_recall_precision_optimized,
...
def main(threshold: float = 0.65,
    ...
    """
    Main training pipeline for fourth view model.
    ...
    """
    print("=" * 70)
    print("Fourth View Model Training Pipeline")
    print("=" * 70)
    ...
    # 1. Load data
    print("\n[1/10] Loading data...")
    train_df, test_df = load_fourth_view_data(sample_size=None)  # Use all data
    
    # 2. Preprocessing
    print("\n[2/10] Preprocessing data...")
    train_df = preprocess_fourth_view_data(train_df, is_train=True)
    test_df = preprocess_fourth_view_data(test_df, is_train=False)
    
    # 3. Feature engineering
    print("\n[3/10] Creating features...")
    train_df = create_all_fourth_view_features(train_df, is_train=True)
    test_df = create_all_fourth_view_features(test_df, is_train=False)
    ...
    if use_feature_selection:
        ...
        from src.fourth_view.models import FourthViewLightGBM
        temp_model = FourthViewLightGBM(n_estimators=200, learning_rate=0.1, verbose=-1)
        ...
        print("\n[5/10] Feature selection: SKIPPED")
    
    # 6. Train model (with optional ensemble and hyperparameter tuning)
    print("\n[6/10] Training model...")
    if use_ensemble:
    ...
    # 7. Evaluate model with multiple thresholds
    ...
    print("\n[8/10] Finding optimal threshold...")
    
    if min_recall > 0:
        # Find threshold with recall >= min_recall and maximum precision
        print(f"Finding threshold with recall >= {min_recall} and maximum precision...")
        optimal_threshold, optimal_metrics = find_threshold_recall_precision_optimized(
            y_val, y_val_pred_proba,
    ...
    # Plot ROC curve
    try:
        plot_roc_curve(y_val, y_val_pred_proba, title="Fourth View Model - ROC Curve")
    except Exception as e:
        print(f"Could not plot ROC curve: {e}")
    
    # Plot Precision-Recall curve
    try:
        plot_precision_recall_curve(y_val, y_val_pred_proba, title="Fourth View Model - Precision-Recall Curve")
    except Exception as e:
    ...
    print("\n[9/10] Making test set predictions...")
    ...
    if missing_features:
        ...
        # Use only features that exist in both train and test
        available_features = [f for f in selected_features if f in test_df.columns]
        print(f"Using {len(available_features)} available features for test predictions")
    ...
    submission_path = project_root / "results" / "submission_fourth_view.csv"
    ...
    print("\n[10/10] Summary")
    print("=" * 70)
    print(f"Model: Fourth View {'Ensemble' if use_ensemble else 'LightGBM'}")
...
if __name__ == "__main__":
    ...
    model, metrics, threshold_results, optimal_threshold = main(
        ...
        use_ensemble=False,  # Set to True to use ensemble (slower but potentially better)
        min_recall=0.9  # Optimize threshold for recall >= 0.9 with max precision
    )
