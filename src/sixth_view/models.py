...
class SixthViewLightGBM:
    """
    LightGBM model optimized for sixth view features.
    Enhanced with optimized capacity for AUC-ROC > 0.95, Recall > 0.9, Precision > 0.5.
    Includes faster hyperparameter tuning support with Optuna/Hyperopt.
    """
    
    def __init__(self, 
                 n_estimators: int = 2000,  # Optimized for 3-4 hour training: reduced from 2500
                 max_depth: int = 9,  # Optimized: reduced from 10 for faster training
                 learning_rate: float = 0.02,  # Increased from 0.01 for faster convergence
                 num_leaves: int = 127,  # Keep moderate capacity
...
def optimize_hyperparameters_optuna(X_train, y_train,
    ...
    def objective(trial):
        params = {
            ...
            'metric': 'auc',
            'boosting_type': 'gbdt',
            'num_leaves': trial.suggest_int('num_leaves', 127, 255),  # Optimized: reduced range for faster tuning (127-255)
            'learning_rate': trial.suggest_float('learning_rate', 0.005, 0.05, log=True),  # Optimized: higher min for faster convergence
            'feature_fraction': trial.suggest_float('feature_fraction', 0.6, 1.0),
            'bagging_fraction': trial.suggest_float('bagging_fraction', 0.6, 1.0),
            'bagging_freq': trial.suggest_int('bagging_freq', 1, 7),
            'min_child_samples': trial.suggest_int('min_child_samples', 10, 100),  # Optimized: higher min for faster training
            'max_depth': trial.suggest_int('max_depth', 8, 14),  # Optimized: reduced range for faster tuning (8-14)
    ...
    study = optuna.create_study(direction='maximize', study_name='sixth_view_optuna')
...
class EnsembleModel:
    ...
    def __init__(self,
                 lgb_model: Optional[SixthViewLightGBM] = None,
        ...
        Parameters
        ----------
        lgb_model : SixthViewLightGBM, optional
...
def train_sixth_view_model(X_train, y_train,
                           X_val=None, y_val=None,
                           ...
                           cv_folds: int = 3) -> SixthViewLightGBM:  # Optimized: reduced from 5 to 3
    """
    Train sixth view model with optional hyperparameter tuning.
    ...
    model = SixthViewLightGBM(**model_params)
...
def train_ensemble_model(X_train, y_train,
    ...
    if use_lgb:
        ...
        lgb_model = train_sixth_view_model(
            X_train, y_train,
            X_val=X_val, y_val=y_val,
    ...
    if weights is None and use_cv_for_weights:
        ...
        if len(models_list) > 0:
            from src.sixth_view.evaluation import compute_ensemble_weights_from_cv
